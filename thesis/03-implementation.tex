...
At first, Python was meant to be used only for building a proof of concept to answer the question of whether writing a solving library on the Solver backend is feasible for the project. The question has been answered affirmatively, and at that moment we were quite happy with the performance aspect. Unfortunately, down the line, after loading more and more complicated Origami models it became apparent that Python's native performance will not cut it. We were worried that a whole rewrite in a more performant language is imminent. After seeking an alternative solution, the fear has been dispelled. The implementation has benefited from a ten-fold increase in performance by using a performance-oriented Python implementation - \tech{PyPy}.
...
A guide creator is a frontend subcomponent allowing Designers to create step-by-step Instructions from a crease pattern.
...
To the right there is a set of configuration sections correspondingly responsible for: \begin{description}
	...
	\item[file] - Setting an Instruction name and description.
\end{description}
...
Until \textit{Save the Guide} step, all data and actions are done on the client-side. When \textit{Save the Guide} action is invoked:

\begin{itemize}
	\item if a user is logged in, a request is sent to the Community backend with all configuration attached, or
...
The community component is built using Django and split into a couple of subapplications:
...
The data model is mostly derived from the built-in Django model which increased the development speed considerably.
\medskip
...
\medskip
All of the dependencies are bound to a specific minor version. Doing so ensures that in case one of the vendors implements a breaking change our application and pipelines do not break.
...
Solver is a library written in python and tested using the \tech{unittest} module. Most of the business logic is covered. 
...
The community component is written in \tech{Python} using \tech{Django} and therefore uses the \tech{unittest} module that is built-in to the Python standard library. All of the public-facing endpoints are fully covered with integration tests verifying that authentication, authorization, and request body handling are properly implemented. The line test coverage is 96\%.

\medskip
Code style abides by the \tech{PEP8} standard.

\medskip
To ensure build reproducibility the community component is enclosed in a Docker image that is automatically deployed to a production environment. 

\begin{figure}[H]
  \caption{Pipeline - Community backend component.}
  \centering
...
Origuide uses Docker images for packaging backend components. Images are pushed to the \tech{Docker Hub} repository where they are publicly available. The frontend build output consists of a set of static files.

\medskip
The backend deployment configuration is specified using \tech{docker-compose} files. 
Because all the environment specific configuration is externalized to \texttt{.env} files, the deployment configuration remains environment agnostic. This makes \tech{Origuide}'s backend deployable using a single command.

\begin{figure}[H]
	\caption{Example configuration of an \textit{.env} file}
	\lstinputlisting{assets/listings/3-8-env-file}
\end{figure}
The backend component can be deployed using a single command.
\begin{figure}[H]
	\caption{An exemplary command that deploys the backend component.}
	\begin{lstlisting}
docker-compose up -d
	\end{lstlisting}
\end{figure}
...
\begin{figure}[H]
	\caption{An exemplary command that deploys the frontend component.}
	\begin{lstlisting}
...
\begin{itemize}
	\item preventing face collision during solving,
	\item detecting face interpenetration after solving and marking which one is on top.
\end{itemize}
