...
Both Instructions and Transitions are represented using \tech{.fold} files which we have extended with information required by the application.
...
The Solver algorithm does not handle face collisions correctly.
...
Normally when we run the Solver for a single step of an Instruction, it would produce around 
...
When the Solver produces a frame, the encoder is responsible for either including it in or excluding it from the resulting Guide.
